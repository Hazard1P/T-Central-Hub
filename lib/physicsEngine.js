...
export function sampleGravityField({ position = [0, 0, 0], sources = [] }) {
  ...
  for (const source of sources) {
    ...
    const pull = (source.mass || 1) / distSq * 0.012;
    acceleration[0] += (dx / dist) * pull;
    acceleration[1] += (dy / dist) * pull;
    acceleration[2] += (dz / dist) * pull;
    const horizonRadius = (source.radius || 1) * 1.4;
    if (dist < horizonRadius * 3) {
      horizonStress += (horizonRadius * 3 - dist) / (horizonRadius * 3);
    }
  }

  return { acceleration, horizonStress: Number(horizonStress.toFixed(4)) };
}
