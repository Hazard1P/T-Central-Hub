  ...
  return (
    <>
      {nodes.slice(0, limit).map((node) => (
        <Billboard key={node.key} position={[node.position[0], node.position[1] + 2.5, node.position[2]]}>
...
export default function CinematicUniverseCanvas({ className = '' }) {
  ...
  return (
    <div className={`cinematic-universe-shell ${className}`.trim()}>
      <Canvas camera={{ position: [0, 8, 28], fov: tier.isMobile ? 50 : 44 }} dpr={tier.dpr} gl={{ antialias: !tier.isMobile }}>
